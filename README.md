...
* Visualization code is currently limited to analyzing single-node systems (optimization of multi-node systems is possible).
...
Each of these files must be formatted in a specific way, with specific properties defined on each row. See [INPUT_FILE_SPECIFICATION.md] for further details. 
